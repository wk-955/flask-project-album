...
from app.models import User, Photo, Tag, Follow, Collect, Comment, Notification
from app.utils import rename_image, resize_image, redirect_back, flash_errors
from app.notifications import push_comment_notification, push_collect_notification, push_follow_notification
...
def search():
    ...
    if q == '':
        flash('输入关键词：如照片,用户或标签.', 'warning')
        return redirect_back()

    category = request.args.get('category', 'photo')
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['ALBUM_SEARCH_RESULT_PER_PAGE']
    if category == 'user':
    ...
    return render_template('main/search.html', q=q, result=result, pagination=pagination, category=category)


@main_bp.route('/uploads/<path:filename>')
def get_image(filename):
    return send_from_directory(current_app.config['ALBUM_UPLOAD_PATH'], filename)
...
def upload():
    if request.method == 'POST' and 'file' in request.files:
        ...
        filename_s = resize_image(f, filename, current_app.config['ALBUM_PHOTO_SIZE']['small'])
        filename_m = resize_image(f, filename, current_app.config['ALBUM_PHOTO_SIZE']['medium'])
        photo = Photo(
            filename=filename,
...
def show_photo(photo_id):
    ...
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['ALBUM_COMMENT_PER_PAGE']
...
def collect(photo_id):
    photo = Photo.query.get_or_404(photo_id)
    if current_user.is_collectting(photo):
        flash('已经收藏', 'info')
        return redirect(url_for('.show_photo', photo_id=photo_id))
...
def uncollect(photo_id):
    photo = Photo.query.get_or_404(photo_id)
    if not current_user.is_collectting(photo):
        flash('未收藏', 'info')
        return redirect(url_for('.show_photo', photo_id=photo_id))
...
def show_notifications():
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['ALBUM_NOTIFICATION_PER_PAGE']
